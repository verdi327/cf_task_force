class Admin::CoachesController < Admin::BaseController
  ...
  def create
    ...
    respond_to do |format|
      if @coach.save
        format.html { redirect_to admin_coach_path(@coach), notice: 'Coach was successfully created.' }
  ...
  def update
    respond_to do |format|
      if @coach.update(coach_params)
        format.html { redirect_to admin_coach_path(@coach), notice: 'Coach was successfully updated.' }
  ...
  private
    ...
    def coach_params
      params.require(:coach).permit(:name, :title, :description, :priority, {images: []})
    end
end
